...
#Introduction
The smartsizer package is designed to assist investigators who are planning sequential, multiple assignment, randomized trials (SMART) in determining the sample size of individuals to enroll. The main goal motivating SMART designs is determination of optimal dynamic treatment regime (DTR). Therefore, investigators are interested in sizing SMARTs based on the ability to screen out DTRs inferior to the best by a specified amount $\Delta_{\mathrm{min}}$ with probability $1-\beta$ while identifying the true best with a specified probability $1-\alpha$. The smartsizer package utilizes the Multiple Comparisons with the Best (MCB) methodology for determining a set of best DTRs while adjusting for multiple comparisons. 

Using smartsizer, investigators may perform power analyses for an arbitrary type of SMART.
